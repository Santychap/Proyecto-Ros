    ...
    @if(auth()->user()->rol === 'admin')
        <div class="filters-container">
            ...
            <div class="admin-actions">
                <form method="POST" action="{{ route('pedidos.vaciarTodos') }}" 
                      onsubmit="return confirm('¿Estás seguro de que quieres eliminar TODOS los pedidos? Esta acción no se puede deshacer.');">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn-danger-large">
                        <i class="fas fa-trash-alt"></i> Vaciar Todos los Pedidos
                    </button>
                </form>
            </div>
        </div>
                    ...
                    <div class="pedido-productos">
                        ...
                        @foreach ($pedido->detalles as $detalle)
                            <div class="producto-item">
                                ...
                                <span class="producto-precio">${{ number_format($detalle->precio * $detalle->cantidad, 2) }}</span>
                            </div>
                        @endforeach
                        <div class="total-pedido">
                            <strong>Total: ${{ number_format($pedido->detalles->sum(function($d) { return $d->precio * $d->cantidad; }), 2) }}</strong>
                        </div>
                    </div>
                </div>
                ...
                @if(Auth::user()->rol !== 'cliente')
                    <div class="pedido-actions">
                        @if(Auth::user()->rol === 'admin' || Auth::user()->rol === 'empleado')
                            ...
                            @if(Auth::user()->rol === 'admin')
                                <form action="{{ route('pedidos.destroy', $pedido) }}" method="POST" class="inline-form" 
