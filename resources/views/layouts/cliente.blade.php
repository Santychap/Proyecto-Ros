...
<head>
    ...
    <title>@yield('title', 'Panel Cliente - Olla y Sazón')</title>
...
<body style="background: linear-gradient(120deg, #fff 60%, #181828 100%); background-attachment: fixed; min-height: 100vh;">
    <header class="admin-header" style="background: #181828; color: #ffd700; padding: 0.5rem 2rem; display: flex; align-items: center; justify-content: space-between; min-height: 56px;">
        <div style="display: flex; align-items: center; gap: 1rem;">
            ...
            <span style="font-size: 1rem; color: #fff; opacity: 0.7; margin-left: 1rem;">Panel de Cliente</span>
        </div>
        <div style="display: flex; align-items: center; gap: 1rem;">
        ...
        <aside class="admin-sidebar" style="width: 220px; background: #181828; color: #ffd700; padding: 2rem 0 2rem 0.5rem; display: flex; flex-direction: column; gap: 1rem;">
            <nav>
                <ul style="list-style: none; padding: 0; margin: 0;">
                    ...
                    <li><a href="{{ route('reservas.index') }}" class="sidebar-link-admin{{ request()->routeIs('reservas.*') ? ' active' : '' }}"><i class="fas fa-calendar-plus"></i> Mis Reservas</a></li>
                    <li><a href="{{ route('menu.index') }}" class="sidebar-link-admin{{ request()->routeIs('menu.*') ? ' active' : '' }}"><i class="fas fa-utensils"></i> Ver Menú</a></li>
                    <li><a href="{{ route('pedidos.index') }}" class="sidebar-link-admin{{ request()->routeIs('pedidos.*') ? ' active' : '' }}"><i class="fas fa-shopping-bag"></i> Mis Pedidos</a></li>
                    <li><a href="{{ route('pagos.historial') }}" class="sidebar-link-admin{{ request()->routeIs('pagos.historial') ? ' active' : '' }}"><i class="fas fa-credit-card"></i> Mis Pagos</a></li>
                    <li><a href="{{ route('profile') }}" class="sidebar-link-admin{{ request()->routeIs('profile') ? ' active' : '' }}"><i class="fas fa-user-cog"></i> Mi Perfil</a></li>
                </ul>
                <style>
                .sidebar-link-admin {
                    display: flex;
